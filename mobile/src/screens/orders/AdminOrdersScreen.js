...
export default function AdminOrdersScreen({ navigation }) {
  ...
  const cancelOrder = (order) => {
    ...
    if (Platform.OS === 'web') {
      ...
      Alert.alert(
        'Cancel and refund?',
        `Order ${order.orderNumber} for ${order.customer?.name} (${formatPrice(order.totalAmount || 0)}) will be cancelled${order.paymentMethod === 'card' ? ' and refunded via Stripe' : ''}. Stock returns to inventory; listings reopen.`,
        [
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.name}>
                      {item.items?.[0]?.gemNameSnapshot || item.items?.[0]?.gem?.name || 'Gem'}
                      {item.items?.length > 1 ? ` + ${item.items.length - 1} more` : ''}
                    </Text>
                    <Text style={styles.customer}>
                      {item.customer?.name} · {formatPrice(item.totalAmount || 0)}
