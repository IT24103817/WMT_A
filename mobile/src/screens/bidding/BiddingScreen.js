...
export default function BiddingScreen({ navigation }) {
  ...
  const closed = list.filter((b) => b.status === 'closed' || b.status === 'cancelled');
  const data = [...active, ...scheduled, ...closed];
                  ...
                  <View style={styles.row}>
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                      <Text style={styles.label}>
                        {item.status === 'active' ? 'Ends in' : item.status === 'scheduled' ? 'Starts' : 'Status'}
                      </Text>
                      {item.status === 'active' ? (
