...
const statusVariant = { active: 'success', closed: 'neutral', cancelled: 'warn' };
        ...
        renderItem={({ item, index }) => (
          <AnimatedListItem index={index}>
            <Card>
              ...
              {item.status === 'active' ? (
                <Button title="Cancel auction" variant="outline" size="sm" onPress={() => remove(item)} style={{ marginTop: 12 }} textStyle={{ color: colors.danger }} />
              ) : null}
            </Card>
          </AnimatedListItem>
        )}
